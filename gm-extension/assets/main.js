      ...
      return Promise.race([
        new Promise((resolve, reject) => {
          ...
          function onResponse(value) {
            self2.off(`${id}_RESPONSE${nonce}`, onResponse);
            self2.off(`${id}_ERROR${nonce}`, onError);
            resolve(value);
          }
...
async function getRoomSceneContext() {
  ...
  const sceneId = String(lib_default.scene?.id ?? "").trim();
  return {
    campaignId: roomId,
    roomId,
    sceneId
...
async function mountBridgeShell({
  ...
  features = {},
  tokenRealtimeSync: tokenRealtimeSync2 = null
}) {
  ...
  const refs = {
    ...
    placementSearch: root2.querySelector('[data-field="placementSearch"]'),
    placementCharacterSelect: root2.querySelector('[data-field="placementCharacterSelect"]'),
    placementInstanceName: root2.querySelector('[data-field="placementInstanceName"]'),
    includeActiveNpc: root2.querySelector('[data-field="includeActiveNpc"]'),
    placementSelectedToken: root2.querySelector('[data-field="placementSelectedToken"]'),
    placementSelectionInfo: root2.querySelector('[data-field="placementSelectionInfo"]'),
    placementHint: root2.querySelector('[data-field="placementHint"]')
  };
  const buttons = {
  ...
  buttons.clearSettings.addEventListener("click", async () => {
    ...
    try {
      ...
      if (tokenRealtimeSync2?.reconcileNow) {
        await tokenRealtimeSync2.reconcileNow("settings-cleared");
      }
      render();
    } catch (error) {
...
function __awaiter21(thisArg, _arguments, P, generator) {
  function adopt(value) {
    return value instanceof P ? value : new P(function(resolve) {
      resolve(value);
    });
...
var FunctionsClient = class {
  ...
  invoke(functionName_1) {
    return __awaiter21(this, arguments, void 0, function* (functionName, options = {}) {
      var _a;
      let timeoutId;
  ...
  _updateFilterTransform() {
    this.channelAdapter.updatePayloadTransform((event, payload, ref) => {
      if (typeof payload === "object" && "ids" in payload) {
        ...
        const { schema, table, commit_timestamp, type, errors: errors2 } = postgresChanges;
...
var { get: get2, post, put, head, remove } = defaultApi;
...
var AuthImplicitGrantRedirectError = class extends CustomAuthError {
  constructor(message, details2 = null) {
...
function __awaiter22(thisArg, _arguments, P, generator) {
  function adopt(value) {
    return value instanceof P ? value : new P(function(resolve) {
      resolve(value);
    });
...
// bridge/realtimeClient.js
var _client = null;
var _url = "";
var _key = "";
function getRealtimeClient(settings) {
...
function buildMetadataPayload(link) {
  return normalizeTokenCharacterLink({
    ...
    updatedAt: link?.state_updated_at ?? link?.updated_at ?? (/* @__PURE__ */ new Date()).toISOString()
  });
}
...
function metadataMatches(current2, next) {
  return current2.characterId === next.characterId && current2.stateVersion === next.stateVersion && current2.statusSummary === next.statusSummary;
}
function createTokenRealtimeSync({ runtime: runtime2 }) {
  const characterApi = runtime2?.api?.character;
  const obrBridge = runtime2?.bridges?.obr;
  const settingsBridge = runtime2?.bridges?.settings;
  const tokenBridge = runtime2?.bridges?.token;
  const state = {
    started: false,
    stopped: false,
    reconcileTimer: 0,
    ...
    combatChannels: /* @__PURE__ */ new Map(),
    linksByTokenId: /* @__PURE__ */ new Map(),
    linksByCharacterId: /* @__PURE__ */ new Map(),
    roomId: "",
    sceneId: "",
    contextKey: "",
    lastSettings: null,
  ...
  function resetLinkMaps() {
    state.linksByTokenId = /* @__PURE__ */ new Map();
    state.linksByCharacterId = /* @__PURE__ */ new Map();
  }
  ...
  async function writeMetadataIfNeeded(token, nextMetadata) {
    const current2 = tokenBridge.getTokenCharacterLink(token);
    if (metadataMatches(current2, nextMetadata)) {
      return false;
    }
  ...
  async function clearOrphanedTokenMetadata(sceneItems, links) {
    ...
    for (const item of safeArray(sceneItems)) {
      ...
      if (!current2.characterId) continue;
      try {
  ...
  async function ensureRealtimeChannels(context, settings) {
    ...
    if (!state.tokenLinksChannel) {
      state.tokenLinksChannel = client.channel(`odyssey:token-links:${nextContextKey}`).on(
        "postgres_changes",
        {
          event: "*",
          schema: "public",
    ...
    for (const characterId of activeCharacterIds) {
      ...
      const channel = client.channel(`odyssey:combat-state:${nextContextKey}:${characterId}`).on(
        "postgres_changes",
        {
          event: "*",
          schema: "public",
  ...
  async function reconcileNow(reason = "manual") {
    ...
    try {
      ...
      ).filter((link) => link?.is_active !== false) : activeLinks;
